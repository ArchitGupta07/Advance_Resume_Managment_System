...
from parser.new_location import extract_locations_from_text, all_locs
from parser.workex_duration import toExtract_Experience_Rolewise
...
def toCheckAcutualPhonenumbers(data):
  lst=[]
  for i in data:
    if re.match(r'\d{10}\s+',i):
      i = i[:10]
    i = re.sub(r'\s+', '', i)
    if len(i)==12:
      if i.startswith("91"):
      ...
      if i.startswith("0"):
        lst.append(i[1:])
  return lst
def to_extract_phone_number(new_text,status=True):
  text = re.sub(r"[^A-Za-z0-9]"," ",new_text)
  text = re.sub(r'\s+', ' ', text)
  pattern = r'\b(?:\d{3,5}[\s-]*?\d{3,5}[\s-]*?\d{3,5})|\b(?:\d{5}[\s-]*?\d{5}[\s-]*?)'
  # pattern = r'\b(?:\d{3,5}[\s-]*?\d{3,5}[\s-]*?\d{3,5})\b'
  phone_Number = re.findall(pattern,text)
  phone_Number = toCheckAcutualPhonenumbers(phone_Number)
  if len(phone_Number)==0 and status:
    text = re.sub(r'\b[A-Z ]+\b|\b[a-z]\s+\b', combine_words, new_text)
    to_extract_phone_number(text,False)
  return phone_Number
...
def extract_prev_job_roles(text):
    ...
    {"POS": {"IN": ["PROPN", "NOUN","VERB"]}, "DEP": {"IN": ["compound", "nsubj"]}, "OP": "*"},
    {"POS": {"IN": ["PROPN", "NOUN","VERB"]}, "DEP": {"IN": ["compound", "nsubj","appos","dep", "ROOT","nmod","pobj","dobj"]}, "OP": "*"}
]
    ...
    for match in longest_matches:
        ...
        for role in job_roles:
            if role in check_role.split() :
                ...
                j_r = " ".join(role_arr[role_arr.index(role)-2:role_arr.index(role)+1]) if role_arr.index(role)!=0 and role_arr.index(role)>2 and  len(role_arr)>3 else " ".join(role_arr[:3])
...
def extract_work_ex(text):
    ...
    dates = extract_date2(text)
    cities  = all_locs(re.sub(r"[\[\](){}<>\"'`_]", " ", text))
    # print(job_roles)
    # print(dates)
    # print(cities)
    # print(text)
...
def summary_parser(text):
    ...
    text = join_split_words(text).replace("  "," ").lower().title()  
...
def summary_exp_duration(text):
    try:
        ...
        exp_pattern = r"(?i)((\d.?\d?\d?)\s?(\+|plus)?\s?(years|year|yrs))"
        exp = 0
        matches = re.findall(exp_pattern, text)
...
from parser.Font_extractor import get_name, download_pdf
...
# url ="D:\Ex2_Projects\Resume_parser\Resumes\demo.docx"
    try:
        ...
        emergency_text = tika_text_extraction(filePath).strip()
    ...
    # print(text)
        ...
        if not emergency_text:
            print("no emergency text found", filePath)

        # print(divide_resume_sections(emergency_text)["Experience"])
        new_work_data = toExtract_Experience_Rolewise(divide_resume_sections(emergency_text)["Experience"])
        # if new_work_data["total"] == 0:
        #     new_work_data = toExtract_Experience_Rolewise(emergency_text)
            
        curr_role = ""
        # print(divide_resume_sections(emergency_text)["Experience"])
        print("new work data",new_work_data)

        if new_work_data["total"]!=0:
            for k,v in new_work_data.items():
        ...
        try:
            data = {
                ...
                "Summary":summary_parser(emergency_text.strip()),        
                "Name": process_get_name(filePath,text, emergency_text),
                "Phn_no": to_extract_phone_number(emergency_text),
                ...
                # "Locations":extract_locations2(emergency_text)
                "Locations":extract_locations_from_text(emergency_text),
                "Curr_role":curr_role,
                "TotalExp":summary_exp_duration(emergency_text) if summary_exp_duration(emergency_text) else new_work_data["total"]
            }
    ...
    # =======
        ...
        try:
            ...
            print('entered the url try block!!*****')
            # url = "http://localhost:8000/alldata"
            url = "https://armss-be.exitest.com/alldata"
            # url = "https://armss-be.exitest.com/upload/alldata"
            # url = "http://127.0.0.1:8001/upload/alldata"
