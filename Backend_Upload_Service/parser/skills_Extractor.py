...
# print(content.split(','))
    ...
    final_skills = [i.capitalize() for i in set([i.lower() for i in skillset]) if i.strip()]
...
def add_newskills(newSkills):
    ...
    try:
        ...
        for new_skill in newSkills:
            if new_skill.lower().replace(" ","") not in existing_skills:
                existing_skills.append(new_skill.lower().replace(" ",""))
