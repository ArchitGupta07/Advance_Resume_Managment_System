...
def projects_extraction(text):
    ...
    for proj in project_names: 
        ...
        elif re.match(r'\d?\d\/20\d\d\s*-\s*\d?\d\/20\d\d', proj):
            project_names.remove(proj)
            # continue
            # print("B")
        elif len(proj.split("/"))>1:
            project_names.remove(proj)
        elif "certiﬁcate" in list(proj.lower().split()) or "certificate" in proj.lower().split():
            # print("certi")
            project_names.remove(proj)
...
# text = ""
# for page in extract_text_from_pdf("D:/Ex2_Projects/TalenTrack/api/data/resumes/resume17.pdf"):
